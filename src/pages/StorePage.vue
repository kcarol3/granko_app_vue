            ...
            <div class="store-page">
                <div class="image-grid">
                    <div v-for="(image, index) in filteredImages"
                         :key="index" class="image-item" @click="goToGamePage(image.id)">
                        <!--            <img :src="image.src" :alt="image.alt || `Image ${index + 1}`" class="image"/>-->
                        <div class="image">
                            <h2>{{ image.title }}</h2>
                            <h3>{{ image.subTitle }}</h3>
                            <h4>Price: {{ image.price }} USD</h4>
                            <h4>type: {{ image.type }} USD</h4>
                        </div>
                    </div>
                </div>
            </div>
...
<!--&lt;!&ndash;                        <option value="priceAsc">Price: Low to High</option>&ndash;&gt;-->
<!--&lt;!&ndash;                        <option value="priceDesc">Price: High to Low</option>&ndash;&gt;-->
...
export default {
    ...
    data() {
        return {
            ...
            categories: ["RPG", "ADVENTURE", "SCIFI"],
    ...
    methods: {
        getGames() {
            ...
            if (token) {
                this.$axios.get("http://127.0.0.1:8080/games", {
                    headers: {
        ...
        sortGames() {
            if (this.selectedSortOption === "titleAsc") {
...
@media (min-width: 768px) {
    .image-grid {
        grid-template-columns: repeat(2, 1fr); /* 2 kolumny dla ekranów średnich */
    }
}

@media (min-width: 1024px) {
    .image-grid {
        grid-template-columns: repeat(3, 1fr); /* 3 kolumny dla ekranów dużych */
    }
}
